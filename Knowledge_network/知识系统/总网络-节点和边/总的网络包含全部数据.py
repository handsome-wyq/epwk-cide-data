...
# 4）总输出目录
global_output_dir = os.path.join(main_base_dir, "总网络-节点和边")
...
def load_main_orders():
    ...
    返回 df_main_all：
    至少包含列：
    ['任务编号', '发布时间', '需求方', '设计师', '能力标签', '需求文本', '标题', '交易价格', ...]
    """
    all_dfs = []
...
def build_main_nodes_and_edges_global(df_main_all, map_demander_id, map_designer_id):
    ...
    for d in demanders:
        ...
        subject_nodes.append([sid, d, '需求方', 3])

    for d in designers:
        sid = map_designer_id(d)
        subject_nodes.append([sid, d, '设计师', 3])

    # 能力节点
    if '能力标签' in df_main_all.columns:
        for ab in df_main_all['能力标签'].dropna().unique():
            ability_nodes.append([ab, ab, '能力', 2])

    # 订单节点（全局）
    if '任务编号' in df_main_all.columns:
        orders_df = df_main_all[['任务编号', '发布时间']].dropna(subset=['任务编号'])
        orders_df = orders_df.sort_values(by='发布时间').drop_duplicates(subset=['任务编号'], keep='first')
        for _, row in orders_df.iterrows():
            oid = row['任务编号']
            pub_str = row['发布时间'].strftime('%Y-%m-%d') if pd.notna(row['发布时间']) else None
            order_nodes.append([oid, pub_str, '订单', 1])

    # 边：主体 -> 能力
    tmp = df_main_all.dropna(subset=['能力标签'])
    for _, row in tmp.iterrows():
        ab = row['能力标签']
        dem_name = row['需求方']
        des_name = row['设计师']

        if pd.notna(dem_name):
            src_id = map_demander_id(dem_name)
            edges_sa.append([src_id, ab, '发布'])
    ...
    # 边：能力 -> 订单
    tmp2 = df_main_all.dropna(subset=['能力标签', '任务编号'])
    ...
    df_order   = pd.DataFrame(order_nodes,   columns=['ID', 'Publication_Time', 'type', '[z]']).drop_duplicates()
    ...
    return df_subject, df_ability, df_order, df_edges_sa, df_edges_ao
...
def build_main_nodes_and_edges_for_year(year, df_main_all, map_demander_id, map_designer_id):
    ...
    if df_y.empty:
        ...
        return empty_sub, empty_ab, empty_ord, empty_es, empty_ea
    ...
    for _, row in df_y.iterrows():
        ...
        if pd.notna(dem):
            ...
            subject_nodes.append([sid, dem, '需求方', 3])
            if pd.notna(ab) and ab != "":
                edges_sa.append([sid, ab, '发布'])

        if pd.notna(des):
            sid = map_designer_id(des)
            subject_nodes.append([sid, des, '设计师', 3])
            if pd.notna(ab) and ab != "":
    ...
    if order_nodes:
        ...
        df_order = pd.DataFrame(columns=['ID', 'Publication_Time', 'type', '[z]'])
...
def load_main_knowledge_for_year(year):
    ...
    从 {year}.json 中提取：
      ...
      - 订单->知识 边
      - 知识->知识 关系边（relations）
      - JSON 中出现的订单节点（ID, Publication_Time）
    """
    ...
    for line in lines:
        ...
        pub_str = parse_date_to_str(record.get('发布时间'))
        order_nodes.append([task_id, pub_str, '订单', 1])
    ...
    df_ord = pd.DataFrame(order_nodes, columns=['ID', 'Publication_Time', 'type', '[z]']).drop_duplicates()
...
def load_main_knowledge_global():
    ...
    for year in years:
        df_k, df_ok, df_kk, df_ord = load_main_knowledge_for_year(year)
        if not df_k.empty:
            all_k.append(df_k)
        if not df_ok.empty:
            all_ok.append(df_ok)
    ...
    if all_ord:
        df_ord_all = pd.concat(all_ord, ignore_index=True).drop_duplicates()
    else:
...
def process_direct_employment_for_year(year):
    ...
    返回：
      ...
      df_edges_sa_dir, df_edges_ao_dir, df_edges_ok_dir, df_edges_kk_dir
    """
    ...
    if not os.path.exists(orders_path):
        ...
        return empty_sub, empty_ab, empty_ord, empty_k, empty_e, empty_e.copy(), empty_e.copy(), empty_e.copy()
    ...
    for _, row in df_orders.iterrows():
        ...
        if pd.notna(demander):
            subject_nodes.append([demander, demander, '需求方', 3])
        if pd.notna(designer):
            subject_nodes.append([designer, designer, '设计师', 3])

        if pd.notna(ability) and ability != "":
            ability_nodes.append([ability, ability, '能力', 2])
            if pd.notna(demander):
                edges_sa.append([demander, ability, '发布'])
            if pd.notna(designer):
                edges_sa.append([designer, ability, '完成'])
            edges_ao.append([ability, oid, '属于'])

        # 订单节点（无发布时间，用 None）
        order_nodes.append([oid, None, '订单', 1])
    ...
    return df_sub, df_ab, df_ord, df_k, df_es, df_ea, df_eo, df_ek
...
def load_direct_employment_global():
    ...
    for year in years:
        (df_sub_y, df_ab_y, df_ord_y, df_k_y,
         df_es_y, df_ea_y, df_eo_y, df_ek_y) = process_direct_employment_for_year(year)
    ...
    df_ord_all = cat_or_empty(all_ord, ['ID', 'Publication_Time', 'type', '[z]'])
    ...
    return df_sub_all, df_ab_all, df_ord_all, df_k_all, df_es_all, df_ea_all, df_eo_all, df_ek_all
...
def build_global_and_yearly_network():
    ...
    # 主平台：全局 主体/能力/订单 + 边
    df_sub_main, df_ab_main, df_ord_main, df_es_main, df_ea_main = \
        build_main_nodes_and_edges_global(df_main_all, map_demander_id, map_designer_id)

    # ② 主平台JSON知识：全局
    df_k_json, df_ok_json, df_kk_json, df_ord_json = load_main_knowledge_global()

    # ③ 直接雇佣：全局
    (df_sub_dir, df_ab_dir, df_ord_dir, df_k_dir,
     df_es_dir, df_ea_dir, df_eo_dir, df_ek_dir) = load_direct_employment_global()
    # ---------- 给全局边加上统一的 EdgeType（边的大类） ----------

    # 主体 -> 能力
    if not df_es_main.empty:
        df_es_main['EdgeType'] = df_es_main['Type'].map(
            {'发布': '需求方-能力', '完成': '设计师-能力'}
        ).fillna('主体-能力')  # 其它情况兜底为“主体-能力”

    if not df_es_dir.empty:
        df_es_dir['EdgeType'] = df_es_dir['Type'].map(
            {'发布': '需求方-能力', '完成': '设计师-能力'}
        ).fillna('主体-能力')
    ...
    # 订单层（主平台 + 直接雇佣 + JSON中的订单）
    df_orders_all = pd.concat([df_ord_main, df_ord_dir, df_ord_json], ignore_index=True).drop_duplicates(subset=['ID', 'type'])

    # 知识层（JSON + 直接雇佣）
    df_knowledge_all = pd.concat([df_k_json, df_k_dir], ignore_index=True).drop_duplicates()

    # ---------- 全局边合并 ----------
    df_edges_sa_all = pd.concat([df_es_main, df_es_dir], ignore_index=True).drop_duplicates()  # 主体-能力
    ...
    df_edges_sa_all.to_csv(os.path.join(global_output_dir, "edges_subject_ability_all.csv"),
                           index=False, encoding='utf-8-sig')
    ...
    print(f"主体-能力边: {len(df_edges_sa_all)}")
    ...
    for year in years:
        ...
        # 1）主平台：当年的子网
        (df_sub_main_y, df_ab_main_y, df_ord_main_y,
         df_es_main_y, df_ea_main_y) = build_main_nodes_and_edges_for_year(year, df_main_all,
                                                                            map_demander_id, map_designer_id)

        # 2）JSON 知识：当年
        df_k_json_y, df_ok_json_y, df_kk_json_y, df_ord_json_y = load_main_knowledge_for_year(year)

        # 3）直接雇佣：当年
        (df_sub_dir_y, df_ab_dir_y, df_ord_dir_y, df_k_dir_y,
         df_es_dir_y, df_ea_dir_y, df_eo_dir_y, df_ek_dir_y) = process_direct_employment_for_year(year)
        ...
        # 订单
        df_orders_y = pd.concat([df_ord_main_y, df_ord_dir_y, df_ord_json_y],
        ...
        if not df_edges_sa_y.empty:
            df_edges_sa_y['EdgeType'] = df_edges_sa_y['Type'].map(
                {'发布': '需求方-能力', '完成': '设计师-能力'}
            ).fillna('主体-能力')
        ...
        df_edges_sa_y.to_csv(os.path.join(year_dir, f"edges_subject_ability_{year}.csv"),
                             index=False, encoding='utf-8-sig')
