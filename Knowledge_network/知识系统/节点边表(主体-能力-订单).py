...
output_dir_base_main = os.path.join(base_dir, "节点边表(主体-能力-订单)")  # 输出根目录
...
def generate_nodes_and_edges(all_df, output_dir, year=None):
    ...
    if all(col in all_df.columns for col in ['需求方', '设计师']):
        # 获取唯一需求方和设计师
        unique_demands = set(all_df['需求方'].dropna().unique())
        unique_designers = set(all_df['设计师'].dropna().unique())

        # 找出重复的名称（既是需求方又是设计师）
        duplicates = unique_demands.intersection(unique_designers)

        # 需求方节点
        demanders = all_df['需求方'].dropna().unique()
        demanders_df = pd.DataFrame({'ID': demanders, 'Name': demanders, 'Type': '需求方'})
        demanders_df['ID'] = demanders_df.apply(
            lambda row: 'A' + row['Name'] if row['Name'] in duplicates else row['Name'], axis=1
        )
        demanders_df['[z]'] = 3  # 添加 [z] 列，值为3

        # 设计师节点
        designers = all_df['设计师'].dropna().unique()
        designers_df = pd.DataFrame({'ID': designers, 'Name': designers, 'Type': '设计师'})
        designers_df['ID'] = designers_df.apply(
            lambda row: 'B' + row['Name'] if row['Name'] in duplicates else row['Name'], axis=1
        )
        designers_df['[z]'] = 3  # 添加 [z] 列，值为3
    ...
    if all(col in all_df.columns for col in ['任务编号', '交易价格', '发布时间']):
        ...
        orders_df['type'] = '订单'  # 添加 type 列，值为 '订单'
        orders_df['[z]'] = 1  # 添加 [z] 列，值为2
    ...
    if '能力标签' in all_df.columns:
        ...
        abilities_df['type'] = '能力'  # 添加 type 列，值为 '能力'
        abilities_df['[z]'] = 2  # 添加 [z] 列，值为1
    ...
    # 4. 边表：主体到能力标签
    if all(col in all_df.columns for col in ['需求方', '设计师', '能力标签', '交易价格', '发布时间']):
        # 需求方到能力标签的边
        edges_demand_ability = all_df[['需求方', '能力标签', '交易价格', '发布时间']].dropna()
        edges_demand_ability['Type'] = '发布'
        edges_demand_ability.columns = ['Source', 'Target', 'Transaction_Price', 'Publication_Time', 'Type']
        edges_demand_ability['Source'] = edges_demand_ability.apply(
            lambda row: 'A' + row['Source'] if row['Source'] in duplicates else row['Source'], axis=1
        )

        # 设计师到能力标签的边
        edges_design_ability = all_df[['设计师', '能力标签', '交易价格', '发布时间']].dropna()
        edges_design_ability['Type'] = '完成'
        edges_design_ability.columns = ['Source', 'Target', 'Transaction_Price', 'Publication_Time', 'Type']
        edges_design_ability['Source'] = edges_design_ability.apply(
            lambda row: 'B' + row['Source'] if row['Source'] in duplicates else row['Source'], axis=1
        )

        # 合并主体到能力标签的边
        edges_subject_ability = pd.concat([edges_demand_ability, edges_design_ability], ignore_index=True)
        edges_sa_path = os.path.join(output_dir, f"{prefix}edges_subject_ability.csv")
        edges_subject_ability.to_csv(edges_sa_path, index=False, encoding='utf-8-sig')
        print(f"主体到能力标签边表已保存到: {edges_sa_path}")
    else:
        print("警告: 缺少必要的列，无法生成主体到能力标签边。")

    # 5. 边表：能力标签到订单
    if all(col in all_df.columns for col in ['能力标签', '任务编号']):
        edges_ability_order = all_df[['能力标签', '任务编号']].dropna()
        edges_ability_order.columns = ['Source', 'Target']
        edges_ability_order['Type'] = '属于'
        edges_ao_path = os.path.join(output_dir, f"{prefix}edges_ability_order.csv")
        edges_ability_order.to_csv(edges_ao_path, index=False, encoding='utf-8-sig')
        print(f"能力标签到订单边表已保存到: {edges_ao_path}")
    else:
        print("警告: 缺少必要的列，无法生成能力标签到订单边。")
